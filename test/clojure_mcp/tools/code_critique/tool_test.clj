(ns clojure-mcp.tools.code-critique.tool-test
  (:require [clojure.test :refer :all]
            [clojure-mcp.tools.code-critique.tool :as tool]
            [clojure-mcp.config :as config]
            [clojure-mcp.agent.langchain.model :as model]))

(deftest test-create-code-critique-tool-with-config
  (testing "Tool creation with model configuration"
    (binding [model/*env-overrides* {"ANTHROPIC_API_KEY" "test-key"}]
      (let [nrepl-client-atom (atom {::config/config
                                     {:tools-config {:code_critique {:model :anthropic/claude-3-haiku-20240307}}
  ...
  (testing "Tool creation with explicit model overrides config"
    (binding [model/*env-overrides* {"ANTHROPIC_API_KEY" "test-key"}]
      (let [nrepl-client-atom (atom {::config/config
        ...
        (let [tool-config (tool/create-code-critique-tool nrepl-client-atom explicit-model)]
          (is (= :code-critique (:tool-type tool-config)))
